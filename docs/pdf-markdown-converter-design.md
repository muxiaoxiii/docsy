...
- 不 OCR，不重采样，不改变原 PDF；
- 两遍流式读取临时文本：先识别跨页重复的页首/页尾，再逐页写 Markdown；不把整份文本层和 Markdown 同时放入内存；
- 跨页、固定页首/页尾的相同或数字序列变化文本会被过滤，避免页眉、页脚和页码把扫描件误判为可用文本层；
- 保留页码分隔，方便核对原始页；
...
文件先进入等待列表，用户确认页段后再开始转换。任务卡显示输入类型、页段、当前阶段、已完成页数、警告和输出文件夹。大文件没有固定超时；只有用户点击取消才停止，取消会传递到正在运行的 Poppler 子进程。失败页可按页段重新执行，不重新扫描已完成页面。
