...
| # | 问题 | 严重程度 | 状态 |
|---|------|----------|------|
| 1 | 标记删除后执行删除，页眉未被删除 | P0 | ✅ 已修 |
| 2 | 显示范围功能重新设计 | P0 | ✅ 已改 |
| 3 | 固定文本模式失效 | P0 | ✅ 已修 |
| 4 | 按证据列表名称模式失效 | P0 | ✅ 已修 |
| 5 | 恢复删除标记按钮来历不明 | P1 | ✅ 已删 |
| 6 | quickCleanup 使用硬编码 zone 高度 | P1 | ✅ 已修 |
| 7 | 一键确认跳过低置信度 | P1 | ✅ 已改 |
| 8 | showTotal 不同步模板 | P1 | ✅ 已修 |
| 9 | 页码列表显示不正确 | P1 | ✅ 已修 |
| 10 | 按钮重组 | P2 | ✅ 已改 |
| 11 | 页眉来源选项 | P2 | ✅ 已改 |
| 12 | 导入流程优化 | P2 | ✅ 已改 |
...
1. **快速检测**：`max_pages = 1` + `scan_artifacts = true` → 只扫首页文本 + artifacts
2. **深度检测**：`max_pages = 0` + `scan_artifacts = true` → 扫全部页文本 + artifacts
3. **Doclet 动画修复**：读取页数和检查书签改用 `tauriCallQuiet`，整个导入流程只保留一个 doclet 动画
4. **首页特殊规则**：证据X/对比文件X 序列直接作为候选

### Doclet 动画问题

导入后 doclet 动画出现多次：
1. 读取页数：每个文件调用 `tauriCallSafe` → 每个文件触发一次 doclet（N 次）
2. 检测页眉页脚：手动触发一次 doclet，内部用 `tauriCallQuiet`（1 次）
3. 检查书签：每个文件调用 `tauriCallSafe` → 每个文件触发一次 doclet（N 次）

**修复**：读取页数和检查书签改用 `tauriCallQuiet`，整个导入流程只保留一个 doclet 动画（检测阶段）。

---

## 测试计划

- [x] 标记删除 → 处理 → 检查 _cleaned 文件夹页眉是否被删除
- [x] 固定文本 → 输入自定义文本 → 列表和渲染都显示自定义文本
- [x] 按证据列表名称 → 列表显示"证据1, 证据2..."
- [x] 显示总页数开关关掉后，格式框更新
- [x] 一键保留/一键忽略功能正确
- [x] 立即删除弹窗流程正确
- [x] 页码 per-file 模式显示格式正确
- [x] 首页特殊匹配证据X/对比文件X
- [x] 导入时 doclet 动画只出现一次

## 变更日志

- 2026-08-09 01:00 — 根据用户详细反馈重写变更单
- 2026-08-09 02:00 — 完成全部 12 个问题实施（P0×4, P1×5, P2×3）
