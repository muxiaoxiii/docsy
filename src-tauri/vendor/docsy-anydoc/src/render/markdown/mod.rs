...
pub fn document_to_markdown(doc: &Document) -> String {
    document_to_markdown_with_asset_paths(doc, HashMap::new())
}

/// Render Markdown with local paths for embedded assets.
///
/// The default renderer deliberately keeps embedded images as alt text because a generic
/// caller has nowhere to write bytes. Docsy supplies the paths only after it has created
/// a matching output directory and can therefore preserve image references safely.
