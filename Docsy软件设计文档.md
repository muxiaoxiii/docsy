...
| PDF / 图片 / 视频工具 | PDF 解锁；图片排版成 docx/pdf；视频抽帧、FFmpeg 检测与下载、时间轴叠加 | `PdfUnlock.vue`、`ImagePaddlerView.vue`、`VideoExtractView.vue` |
...
| 模板制作器精确定位 | 当前仍通过 mammoth HTML 选中文本在 docx 纯文本中模糊查找；已补重复位置候选和保存命中校验，连续空格、跨 run 仍需字符级源位置索引 | 高 |
