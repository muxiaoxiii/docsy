...
### 新增
- **证据 PDF 可选无损优化体积**：规则区"优化体积"开关（el-switch，默认开）。开启后：点击处理时先做无损重建（qpdf 按页重建丢弃不可达对象 + 对象流压缩，在现有处理流程之前执行），大体积垃圾文件后续检测/处理显著加快，优化生成 `<原名>（已优化）.pdf` 副本（后缀命名，不影响文件名开头的证据编号解析与排序），原件不动，列表与输出命名仍基于原件；导出时对每个结果文件就地优化收尾（不产生副本，无收益或失败则保留原结果并提示），完成后汇报体积收益。对页树外带大量垃圾对象的文件（如部分第三方拆分件 38MB → 3.5MB）体积可降一个数量级。
