...
- **Markdown ↔ Word 互转**（文档工具新页签）：拖入 `.md`/`.markdown` 转 docx（标题/粗斜删除线/真多级列表/围栏代码块/GFM 表格/超链接/本地图片嵌入缩放），拖入 `.docx`/`.doc` 转 Markdown（样式映射标题、同格式 run 合并、列表还原、GFM 表格、超链接、内嵌图片导出到 `<文件名>_assets/` 并按文中顺序引用）；旧版 `.doc` 经 office_oxide 中转。输出默认保存在原文件旁。
