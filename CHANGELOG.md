...
### 新增
- **证据导入可选无损优化**：规则区提供"导入时自动优化体积"开关（默认关，用户自行开启）；开启后添加 PDF 时自动执行无损结构优化（qpdf 按页重建丢弃不可达对象 + 对象流压缩），对页树外带大量垃圾对象的文件（如部分第三方拆分件 38MB → 3.5MB）体积可降一个数量级；优化生成 `<原名>（已优化）.pdf` 副本（后缀命名，不影响文件名开头的证据编号解析与排序），原件不动。
