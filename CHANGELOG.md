...
### 新增与优化
- **macOS 外部工具终端一键安装**：解决 macOS 环境下 FFmpeg 等外部工具因清单未部署而自动下载失败的问题；支持自动检测系统 Homebrew 状态，若未安装可一键拉起终端安装国内加速源，若已安装则一键调起终端安装 `ffmpeg-full`。
- **全自动免交互确认**：终端安装命令注入 `NONINTERACTIVE=1` 环境变量，自动跳过 `[y/n]` 安全确认提示，全程无须手动确认。
- **时间戳水印支持**：macOS 平台默认安装带 `drawtext` 滤镜支持的 `ffmpeg-full` 完整版，保障视频抽帧时间戳功能开箱即用。
