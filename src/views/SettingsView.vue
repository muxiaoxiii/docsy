<template>
  <div class="settings-view">
    ...
    <el-card shadow="never" class="settings-card">
      ...
      <el-divider />
      <p class="muted">你的自制模板：</p>
      <div class="menu-grid">
        <el-checkbox
          v-for="t in userTemplates"
          :key="`tpl_${t.id}`"
          :model-value="isVisible(`tpl:${t.id}`)"
          @change="(v) => setVisibility(`tpl:${t.id}`, v)"
        >
          {{ t.name }}
        </el-checkbox>
        <span v-if="!userTemplates.length" class="muted">还没有自制模板</span>
      </div>
    </el-card>

    <el-card shadow="never" class="settings-card">
      <template #header>历史与归档</template>
      <el-form label-width="160px" size="default">
        <el-form-item label="生成历史最多保留">
          <el-input-number
            v-model="settings.history_max"
            :min="1"
            :max="500"
            controls-position="right"
            @change="save"
          />
          <span class="muted ml">条（按模板独立计数）</span>
        </el-form-item>
      </el-form>
    </el-card>
...
const MENU_ITEMS = [
  { key: "letter", label: "所函生成（内置）" },
  { key: "template", label: "模板制作" },
  { key: "manage", label: "模板管理" },
...
onMounted(async () => {
  ...
  try {
    ...
    logWarn("settings", "load_user_templates.failed", { error: err });
  }
  try {
