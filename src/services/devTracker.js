...
import { logTrace } from "./appLogger.js";
...
function record(action, target, detail) {
  ...
  logTrace("dev.tracker", action, { seq: entry.seq, target, ...detail });
}
...
export function installDevTracker() {
  ...
  console.log("[DevTracker] 用户操作追踪已安装，session:", _sessionId);
}
...
export function exportDevLog() {
  return _sessionLog.map(e =>
    `[${e.ts}] #${e.seq} ${e.action} | ${e.target} | ${JSON.stringify(e.detail)}`
  ).join("\n");
}
