/**
 * Central composable for TemplateView — holds all state, computed properties,
 * and operations for the template module.
 *
 * Extracted from TemplateView.vue (2651 lines) to separate logic from template.
 */
...
export function useTemplateState() {
  ...
  async function selectSourceDocx() {
    ...
    if (!selected) return
    // Legacy .doc is converted in the backend (office_oxide first, doc2x sidecar fallback).
    // If conversion drops yellow highlights, the subsequent scan reports that clearly.
