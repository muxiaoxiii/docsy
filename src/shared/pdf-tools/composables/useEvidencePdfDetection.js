...
export function useEvidencePdfDetection({
  ...
  async function detectAllHeaderFooter(options = {}) {
    ...
    // Start Doclet — continuous animation for entire detection batch
    const docletId = `detect_batch:${Date.now()}`
    emitOperationEvent('start', 'detect_pdf_header_footer', docletId)
    let success = 0
    let failed = 0
    ...
    try {
      ...
      const runNext = async () => {
        while (queue.length) {
          ...
          const result = await detectFileHeaderFooter(file, options.deep)
  ...
  async function detectFileHeaderFooter(file, deep = false) {
    return tauriCallQuiet('detect_pdf_header_footer', {
      args: {
        inputPath: file.path,
        maxPages: deep ? 100 : DETECTION_SCAN_MAX_PAGES,
