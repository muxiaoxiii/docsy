...
1.0.7 带来图片排版双层智能推荐（导入方案 + 随每页张数/方向变化的布局感知尺寸）、全局缩放独立继承、预览导出同一几何、图文间距滑槽与冲突智能预检；1.0.6 补充大文件分段稳定处理与电子签章风险确认；1.0.5 将 Doclet 的八种动态接入首页互动与完成反馈。
...
- **每页布局一体化**：每页张数（1/2/3/4/6/9/自定义）与网格排列、填充顺序放在一组；无表格模式固定上下排；末页默认保留原网格，可选「末页重新铺满」。
- **双层智能推荐**：导入推荐方案按素材给出方向/张数；当前布局智能推荐跟随每页张数与页面方向重算安全统一宽度（同时受栏宽、格高与宽高比约束），改张数后多图不再沿用失效的 165mm 等旧宽度。
- **全局尺寸与缩放**：智能统一宽度 / 手动宽度 / 适应页面 / 不缩放；全局比例独立存储，新增页自动继承；本页微调即时进入导出，与预览、冲突检测同一状态。
- **标题图文间距**：标题上/下方 + 图文间距滑槽（0–20mm），预览与 PDF/DOCX 导出共用同一 `caption_gap_mm`。
